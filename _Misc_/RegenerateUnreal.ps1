...
#Find Visual Studio Solution
$VSFileName = (Get-ChildItem ($parentDir + "\*.sln")).BaseName;
if($VSFileName -eq $null)
{
	$VSFileName = "solutionnotfound";
}


#Gather directories and files in an array
$paths = New-Object string[] 5;

$paths[0] = $parentDir + "\.vs\";
$paths[1] = $parentDir + "\Binaries\";
$paths[2] = $parentDir + "\DerivedDataCache\";
$paths[3] = $parentDir + "\Intermediate\";
$paths[4] = $parentDir + "\" + $VSFileName + ".sln";
...
#Ask user for deletion if specified
if($ASKFORDELETION)
{
...
#Check if Visual Studio or Unreal Engine is open
$deletingAllowed = 0;
$iteration = 0;
DO
{
	...
	Write-Host "    UE4 Closed $UE4CheckParsed $dots         " -NoNewLine -ForegroundColor $UE4Color;
...
#Generate new visual studio files
$o = new-object -com Shell.Application;
$folder = $o.NameSpace($parentDir)

$UE4FileName = (Get-ChildItem ($parentDir + "\*.uproject")).Name;
$file = $folder.ParseName("$UE4FileName");
