...
void AFGPickup::Tick(float DeltaTime)
{
	...
	const float PulsatingValue = FMath::MakePulsatingValue(GetWorld()->GetTimeSeconds(), 0.65f) * 30.f;
	const FVector NewLocation = CachedMeshRelativeLocation + FVector(0.f, 0.f, PulsatingValue);
	FHitResult Hit;
	MeshComponent->SetRelativeLocation(NewLocation, false, &Hit, ETeleportType::TeleportPhysics);
	MeshComponent->AddRelativeRotation(FRotator(0.f, 20.f * DeltaTime, 0.f), false, &Hit, ETeleportType::TeleportPhysics);
}
